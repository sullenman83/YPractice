...
## Спринт-11
	# Добавлена наблюдаемость сервисов
		Используются следующие инструменты
		- OpenTelemetry собирает метрики сервисов. Он подключен в каждом сервисе. Метрики доступны <адрес сервиса>/metrics
		- Prometheus собирает данные с указанных в файле prometheus.yml адресов и хранит их внутри себя в виде временных рядов. 
			Доступен по адресу http://localhost:9090
		- Jaeger собирает данные о пуути прохождения запроса. Эти данные в Jaeger передает каждый сервис. Натсройки задаются в секции OtlpSettings
			Endpoint - адрес jaeger куда слать данные
			Protocol - название протокола (HttpProtobuf - передача по http, GRPC - передача по gRPC)
			ScheduledDelayMilliseconds - как часто слать данные
			ExporterTimeoutMilliseconds - таймаут ответа от jaeger
			Графический интерфейс достуен по адресу http://localhost:16686
		- Grafana — это специализированная платформа для визуализации, анализа и мониторинга метрик. Получает данные от Prometheus
			Доступна по адресу http://localhost:3000
			Для каждого сервиса настроены дашборды для отображения latency, Throughput, количество запросов в обработке и количество ошибок в процентах от общего количества 
			запросов. Такдже есть отображение метрик на занятую память, процессорное время, частота сбора мусора по поколениям, количество обработанных задач в секунду
			Дашборды лежат в /Provisioning\Dashboards\json_files
